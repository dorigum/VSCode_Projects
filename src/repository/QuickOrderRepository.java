	...
	public List<OrderItem> getLastOrderItems(long memberId) {
		...
		try (Connection conn = DBUtil.getConnection(); PreparedStatement pstmt = conn.prepareStatement(orderSql)) {
			...
			try (ResultSet rs = pstmt.executeQuery()) {
				...
				String itemSql = "SELECT oi.order_item_id, oi.order_id, oi.menu_id, " + "oi.quantity, oi.unit_price, "
						+ "m.menu_name, c.category_name " + "FROM ORDER_ITEM oi "
			...
			System.err.println("퀵오더 조회 중 오류: " + e.getMessage());
		}
		return list;
	}
}